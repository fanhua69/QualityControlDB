...
BEGIN_MESSAGE_MAP(CDlgInputServer, CPropertyPage)
    ...
    ON_NOTIFY(NM_CLICK, IDC_listServerProperty, OnNMClicklistserverproperty)
END_MESSAGE_MAP()
...
void CDlgInputServer::OnNMClicklistserverproperty(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    int n=pNMLV->iItem;
    CStringListItem *pList=NULL;
    pList = (CStringListItem *)m_listServerProperty.GetItemData (n);

    m_sServerName=pList->GetAt(0);
    m_sServerIP=pList->GetAt(1);
    m_sPort=pList->GetAt(2);
    m_sUserName=pList->GetAt(3);
    m_sPassword=pList->GetAt(5);
    UpdateData(false);
}